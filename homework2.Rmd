...
The Mr. Trash dataset contains data about `r nrow(mrtrash_data)` dumpsters, the dates they were surveyed, and their contents. Each dumpster surveyed collected, on average, about `r as.integer(mean(mrtrash_data$cigarette_butts))` cigarette butts, `r as.integer(mean(mrtrash_data$plastic_bottles))` plastic bottles, `r as.integer(mean(mrtrash_data$chip_bags))` (empty) bags of chips, and `r as.integer(median(mrtrash_data$sports_balls))` lost sports balls! The median number of sports balls in a dumpster in 2016 was
`r mrtrash_data %>% filter(year == 2016) %>% summarise(median(sports_balls))` balls -- a tough year for the athletes of Baltimore.

The precipitation dataset contains data about the precipitation in Baltimore over the years 2016 and 2017 (up to August 2017); `r nrow(precip_data)` months' worth of data is represented. The mean value of inches of precipitation from January - August in 2017 was `r precip_data %>% filter(year == "2017") %>% summarize(mean(precipitation))`, and for the whole of 2016 was `r precip_data %>% filter(year == "2016") %>% summarize(mean(precipitation))`.

The total precipitation in 2017, for the months available in the dataset, was `r precip_data %>% filter(year == 2017) %>% summarise(sum(precipitation))` inches.
